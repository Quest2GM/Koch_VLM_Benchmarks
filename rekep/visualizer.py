...
class Visualizer:
    ...
    def show_img(self, rgb):
        cv2.imwrite('filtered_keypoints.png', rgb[..., ::-1])
        input('see press "enter" to close and continue')
        # cv2.waitKey(0)
    ...
    def _get_scene_points_and_colors(self):
        # scene
        cam_obs = {0:{"points": self.env.camera.pixel_to_3d_points(),
                      "rgb": self.env.camera.capture_image("rgb")}}
